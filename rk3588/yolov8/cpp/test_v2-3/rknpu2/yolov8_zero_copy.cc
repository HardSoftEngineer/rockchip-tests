...
static void dump_tensor_attr(rknn_tensor_attr *attr)
{
    ...
    printf("  index=%d, name=%s, n_dims=%d, dims=[%s], n_elems=%d, size=%d, "
           "w_stride=%d, size_with_stride=%d, fmt=%s, type=%s, qnt_type=%s, zp=%d, scale=%f\n",
           attr->index, attr->name, attr->n_dims, dims,
...
int init_yolov8_model(const char *model_path, rknn_app_context_t *app_ctx)
{
    ...
    // --- Input: native attrs (формат NPU) ---
    printf("input tensors (native):\n");
    ...
    for (int i = 0; i < io_num.n_input; i++)
    {
        ...
        ret = rknn_query(ctx, RKNN_QUERY_NATIVE_INPUT_ATTR,
                         &input_native_attrs[i], sizeof(rknn_tensor_attr));
        if (ret != RKNN_SUCC)
        {
            printf("rknn_query fail! ret=%d\n", ret);
            return -1;
    ...
    // uint8 — runtime сам делает нормализацию и квантизацию на NPU
    ...
    for (int i = 0; i < io_num.n_output; i++)
    {
        ...
        ret = rknn_query(ctx, RKNN_QUERY_NATIVE_OUTPUT_ATTR,
                         &output_native_attrs[i], sizeof(rknn_tensor_attr));
        if (ret != RKNN_SUCC)
        {
            printf("rknn_query fail! ret=%d\n", ret);
            return -1;
    ...
    // --- Логические (не-native) attrs — нужны для постпроцессинга ---
...
// Конвертация формата NPU NC1HWC2 → стандартный NCHW для постпроцессинга.
// NC1HWC2 — тайловый формат: каналы разбиты на группы C2 (обычно 16 или 32),
// данные лежат как [batch, C/C2, H, W, C2]. NCHW — обычный [batch, C, H, W].
int NC1HWC2_i8_to_NCHW_i8(const int8_t *src, int8_t *dst,
...
int release_yolov8_model(rknn_app_context_t *app_ctx)
{
    ...
    if (app_ctx->input_native_attrs  != NULL) { free(app_ctx->input_native_attrs);  app_ctx->input_native_attrs  = NULL; }
    if (app_ctx->output_native_attrs != NULL) { free(app_ctx->output_native_attrs); app_ctx->output_native_attrs = NULL; }
    ...
    for (int i = 0; i < app_ctx->io_num.n_input; i++)
    {
        if (app_ctx->input_mems[i] != NULL)
        {
            ...
            if (ret != RKNN_SUCC) { printf("rknn_destroy_mem fail! ret=%d\n", ret); return -1; }
        }
    }
    ...
    if (app_ctx->rknn_ctx != 0)
    {
        ...
        if (ret != RKNN_SUCC) { printf("rknn_destroy fail! ret=%d\n", ret); return -1; }
        app_ctx->rknn_ctx = 0;
    }
    return 0;
}
                           ...
                           infer_timing_t             *timing)
{
    ...
    // ---------------------------------------------------------------
    // Фаза PRE: letterbox → пишем прямо в NPU-буфер (zero copy)
    // Нет malloc — dst_img.virt_addr указывает на input_mems[0]
    // ---------------------------------------------------------------
    auto t0 = clk::now();
    ...
    {
        printf("input_mems buffer is null!\n");
        return -1;
    }
    ...
    // Здесь нет rknn_inputs_set — буфер уже привязан к NPU через rknn_set_io_mem
    // при инициализации. Runtime читает из input_mems[0] напрямую.

    {
        // Блок {} — защита от goto crosses initialization
        ...
        auto t2 = clk::now();   // конец NPU (чистое время на NPU)

        // ---------------------------------------------------------------
        // Фаза POST: NC1HWC2 → NCHW конвертация + DFL + NMS
        //
        // Это CPU-работа специфичная для zero_copy:
        // NPU пишет результаты в тайловом формате NC1HWC2 в output_mems[i].
        // Постпроцессинг ожидает NCHW — делаем перекладку вручную.
        // В обычной версии rknn_outputs_get делает это за нас.
        // ---------------------------------------------------------------
        ...
        for (uint32_t i = 0; i < app_ctx->io_num.n_output; i++)
        {
            ...
            if (app_ctx->is_quant)
            {
                ...
                {
                    memcpy(outputs[i].buf,
                           app_ctx->output_mems[i]->virt_addr,
                           outputs[i].size);
                }
            }
            else
            {
                printf("zero_copy: fp16 not supported\n");
                goto out;
            }
        }
        ...
        {
            auto t3 = clk::now();   // конец POST (включая NC1HWC2 конвертацию)
